...
from sqlalchemy.dialects import cockroachdb
...
class FloatCoercionTest(fixtures.TablesTest, AssertsExecutionResults):
    ...
    __dialect__ = cockroachdb.dialect()
    __backend__ = True
...
class NumericInterpretationTest(fixtures.TestBase):
    ...
    def test_numeric_codes(self):
        # TODO(joey): If we support other drivers, add them here.
        dialects = (cockroachdb.dialect())
...
class TimePrecisionTest(fixtures.TestBase, AssertsCompiledSQL):
    ...
    @testing.only_on('cockroachdb', 'DB specific feature')
    @testing.provide_metadata
    def test_reflection(self):
        metadata = self.metadata
        t1 = Table(
...
class JSONRoundTripTest(fixtures.TablesTest):
    ...
    def _non_native_engine(self, json_serializer=None, json_deserializer=None):
        ...
        if testing.against("cockroachdb+psycopg2") and \
...
class JSONBRoundTripTest(JSONRoundTripTest):
    # FIXME(joey): Not sure if this will work.
