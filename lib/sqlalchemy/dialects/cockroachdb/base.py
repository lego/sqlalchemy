...
import re
import threading
from sqlalchemy.dialects.cockroachdb.stmt_compiler import CockroachCompiler
...
class CockroachDBDialect(PGDialect_psycopg2):
    ...
    def initialize(self, connection):
        # Bypass PGDialect_psycopg2's initialize implementation, which looks at
    ...
    def get_columns(self, conn, table_name, schema=None, **kw):
        ...
        for row in conn.execute('SHOW COLUMNS FROM "%s"."%s"' %
            ...
            if m is None:
                ...
                if type_args:
                    typ = type_class(*[int(s.strip())
                                       for s in type_args.split(',')])
                else:
                    typ = type_class()
            res.append(dict(
                name=name,
    ...
    def get_indexes(self, conn, table_name, schema=None, **kw):
        ...
        for name in uniques:
            res.append(
                dict(name=name, column_names=columns[name], unique=uniques[name]))
        return res
    ...
    def do_rollback_to_savepoint(self, connection, name):
        ...
        if savepoint_state.cockroach_restart:
            ...
            super(CockroachDBDialect, self).do_rollback_to_savepoint(
                connection, name)
    ...
    def do_release_savepoint(self, connection, name):
        ...
        if savepoint_state.cockroach_restart:
            ...
            super(CockroachDBDialect, self).do_release_savepoint(
                connection, name)
